  ...
  return <main className="create-page">
    ...
    <div className="create-layout">
      <section className="editor-panel">
        ...
        {selectedHero && <div className="form-section"><h2>Hero 자유 꾸미기</h2><p style={{fontSize:12,color:"#8c7468",marginBottom:12}}>기본 디자인 그대로 사용하거나 문구를 추가하고 각 문구의 글꼴·색상·크기·위치를 바꿔보세요. 빈 문구는 표시되지 않아요.</p>{[...(selectedHero.config?.textLayers||[]),...(invitation.heroExtraTextLayers||[])].map((layer,index)=>{const override=invitation.heroLayerOverrides?.[layer.id]||{};const value=(key)=>override[key]??layer[key];const change=(key,next)=>setInvitation(current=>({...current,heroLayerOverrides:{...current.heroLayerOverrides,[layer.id]:{...current.heroLayerOverrides?.[layer.id],[key]:next}}}));return <div key={layer.id} style={{border:"1px solid #eadfd8",borderRadius:10,padding:12,marginBottom:12,display:"grid",gap:9}}><strong>문구 {index+1}</strong><textarea aria-label={`Hero 문구 ${index+1}`} rows={2} maxLength={200} value={typeof invitation.heroTextOverrides?.[layer.id]==="string"?invitation.heroTextOverrides[layer.id]:layer.text||""} onChange={event=>setInvitation(current=>({...current,heroTextOverrides:{...current.heroTextOverrides,[layer.id]:event.target.value}}))}/><Field label="글꼴"><select value={value("fontId")||"great-vibes"} onChange={event=>change("fontId",event.target.value)}>{HERO_FONTS.map(font=><option key={font.id} value={font.id}>{font.label}</option>)}</select></Field><Field label={`크기 ${value("fontSize")??32}px`}><input type="range" min="8" max="100" value={value("fontSize")??32} onChange={event=>change("fontSize",Number(event.target.value))}/></Field><Field label="글씨 색상"><input type="color" value={/^#[0-9a-f]{6}$/i.test(value("color")||"")?value("color"):"#ffffff"} onChange={event=>change("color",event.target.value)}/></Field><Field label={`좌우 위치 ${value("x")??50}%`}><input type="range" min="0" max="100" value={value("x")??50} onChange={event=>change("x",Number(event.target.value))}/></Field><Field label={`상하 위치 ${value("y")??50}%`}><input type="range" min="0" max="100" value={value("y")??50} onChange={event=>change("y",Number(event.target.value))}/></Field><Field label="정렬"><select value={value("align")||"center"} onChange={event=>change("align",event.target.value)}><option value="left">왼쪽</option><option value="center">가운데</option><option value="right">오른쪽</option></select></Field><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><button type="button" onClick={()=>setInvitation(current=>({...current,heroTextOverrides:Object.fromEntries(Object.entries(current.heroTextOverrides||{}).filter(([id])=>id!==layer.id)),heroLayerOverrides:Object.fromEntries(Object.entries(current.heroLayerOverrides||{}).filter(([id])=>id!==layer.id))}))}>이 문구 초기화</button>{(invitation.heroExtraTextLayers||[]).some(item=>item.id===layer.id)&&<button type="button" onClick={()=>setInvitation(current=>({...current,heroExtraTextLayers:(current.heroExtraTextLayers||[]).filter(item=>item.id!==layer.id)}))}>추가 문구 삭제</button>}</div></div>})}<button type="button" onClick={()=>setInvitation(current=>({...current,heroExtraTextLayers:[...(current.heroExtraTextLayers||[]),{id:`extra-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,text:"새 문구",fontId:"great-vibes",fontSize:32,color:"#ffffff",x:50,y:50,align:"center"}]}))}>+ 문구 추가</button><button type="button" style={{marginLeft:8}} onClick={()=>setInvitation(current=>({...current,heroTextOverrides:{},heroLayerOverrides:{},heroExtraTextLayers:[]}))}>Hero 문구 전체 초기화</button></div>}
