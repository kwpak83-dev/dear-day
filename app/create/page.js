...
const initialInvitation = { eventKind: "wedding", templateId: "", heroPresetId: "", heroTextOverrides: {}, rsvpEnabled: true, guestbookEnabled: true, eventTitle: "", hostName: "", person1Name: "", person2Name: "", childName: "", parent1Name: "", parent2Name: "", birthDate: "", dueDate: "", age: "", anniversaryYears: "", organizationName: "", programName: "", coverPhotoUrl: "", 
groom: "",
groomFatherName: "",
...
export default function CreateInvitation() {
  ...
  const composedTemplateRender=selectedHero?{config:templateRender.config?{...templateRender.config,hero:{...templateRender.config.hero,...selectedHero.config,textLayers:(selectedHero.config?.textLayers||[]).map((layer)=>({...layer,text:typeof invitation.heroTextOverrides?.[layer.id]==="string"?invitation.heroTextOverrides[layer.id]:layer.text})),frameAssetId:selectedHeroFrame?.id||null}}:templateRender.config,assets:selectedHeroFrame?.url?{...templateRender.assets,[selectedHeroFrame.id]:selectedHeroFrame.url}:templateRender.assets}:templateRender;
  ...
  return <main className="create-page">
    ...
    <div className="create-layout">
      <section className="editor-panel">
        ...
        {selectedHero && Array.isArray(selectedHero.config?.textLayers) && selectedHero.config.textLayers.length > 0 && <div className="form-section"><h2>Hero 문구 편집</h2><p style={{fontSize:12,color:"#8c7468",marginBottom:12}}>디자인과 글씨체는 유지하고, 이 초대장에 표시할 문구만 바꿀 수 있어요.</p>{selectedHero.config.textLayers.map((layer,index)=><Field key={layer.id} label={`문구 ${index+1}`}><textarea rows={2} maxLength={200} value={typeof invitation.heroTextOverrides?.[layer.id]==="string"?invitation.heroTextOverrides[layer.id]:layer.text||""} onChange={(event)=>setInvitation((current)=>({...current,heroTextOverrides:{...current.heroTextOverrides,[layer.id]:event.target.value}}))}/></Field>)}</div>}
