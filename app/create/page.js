...
const initialInvitation = { eventKind: "wedding", templateId: "", coverPhotoUrl: "", groom: "경원", bride: "보람", date: "2026-10-17", time: "12:30", venue: "더가든 웨딩홀 · 그랜드룸", venueAddress: "", groomBank: "", groomAccount: "", groomAccountHolder: "", brideBank: "", brideAccount: "", brideAccountHolder: "", message: "서로의 모든 날을 함께하기로 약속한 저희,\n소중한 분들을 모시고 첫걸음을 내딛고자 합니다." };
  ...
  return <main className="create-page">
    ...
    <div className="create-layout">
      <section className="editor-panel">
        ...
        <div className="form-section"><h2>행사 종류</h2><Field label="초대장 종류"><select value={invitation.eventKind} onChange={(e) => update("eventKind", e.target.value)}>{eventKindOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field></div>
        <div className="form-section"><h2>템플릿 <small>개발용</small></h2><Field label="디자인"><select value={invitation.templateId} onChange={(e) => update("templateId", e.target.value)} disabled={!templateOptions.length}>{templateOptions.length ? templateOptions.map((template) => <option key={template.id} value={template.id}>{template.name}</option>) : <option value="">템플릿을 불러오는 중이에요</option>}</select></Field>{templateNotice && <p className="photo-notice" role="status">{templateNotice}</p>}</div>
