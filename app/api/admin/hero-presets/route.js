...
function validConfig(value){
  ...
  return heroModes.has(value.mode)&&heroRatios.has(value.aspectRatio)&&decimal(value.positionX,0,100)&&decimal(value.positionY,0,100)&&decimal(value.textYPercent,0,100)&&Number.isInteger(value.scheduleFontSize)&&decimal(value.scheduleFontSize,8,24)&&decimal(value.zoom,.5,2)&&hex(value.overlayColor)&&decimal(value.overlayOpacity,0,1)&&typeof value.headerVisible==="boolean"&&typeof value.mastheadVisible==="boolean"&&typeof value.mastheadText==="string"&&value.mastheadText.length<=60&&value.display&&Object.keys(value.display).length===heroDisplayKeys.length&&heroDisplayKeys.every((key)=>typeof value.display[key]==="boolean")&&(value.textLayers===undefined||(Array.isArray(value.textLayers)&&value.textLayers.length<=12&&value.textLayers.every((layer)=>layer&&typeof layer==="object"&&!Array.isArray(layer)&&Object.keys(layer).every((key)=>["id","text","fontId"].includes(key))&&typeof layer.id==="string"&&/^[a-zA-Z0-9_-]{1,80}$/.test(layer.id)&&typeof layer.text==="string"&&layer.text.length<=200&&typeof layer.fontId==="string"&&["noto-serif-kr","nanum-myeongjo","gowun-batang","gowun-dodum","noto-sans-kr","playfair-display","cormorant-garamond","great-vibes","allura","alex-brush","parisienne","dancing-script"].includes(layer.fontId)&&new Set(value.textLayers.map((layer)=>layer.id)).size===value.textLayers.length));
}
