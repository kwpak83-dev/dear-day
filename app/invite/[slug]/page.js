...
export default async function InvitationPage({ params }) {
  ...
  return <main className="public-invitation"><p>WEDDING INVITATION</p><div className="public-flower">✿ &nbsp; ❋ &nbsp; ✿</div><div className="public-photo">{invitation.coverPhotoUrl ? <img src={invitation.coverPhotoUrl} alt={`${invitation.groom || "신랑"} · ${invitation.bride || "신부"}의 대표사진`} /> : <>{invitation.groom?.slice(0, 1)} &amp; {invitation.bride?.slice(0, 1)}</>}</div><h1>{invitation.groom} <em>&amp;</em> {invitation.bride}</h1><time>{date}</time><hr /><strong>{invitation.venue}</strong>{address && <span>{address}</span>}<blockquote>{invitation.message}</blockquote><Gallery photos={galleryPhotos} /><AccountCopy invitation={invitation} /><footer>디어데이와 함께하는 소중한 순간</footer></main>;
}
